...
namespace JGO {
    ...
    export class Board {
        ...
        toString(c: Coordinate) {
            return COORDINATES[c.i] + (this.height - c.j);
        }
        ...
        clear() {
            this.each(function (c) {
                this.setType(c, CLEAR);
                this.setMark(c, MARK.NONE);
            }.bind(this));
        }
         ...
         */
        setType(c: Coordinate | Coordinate[], t) {
         ...
         */
        getMark(c: Coordinate | Coordinate[]) {
            var ret;
         ...
         */
        getGroup(coord: Coordinate, overrideType?: number) {
         ...
         */
        public playMove(coord, stone, ko: Coordinate): MoveResult {
            var oppType = (stone == BLACK ? WHITE : BLACK),
                captures: Coordinate[] = [], captured = {};

            if (!coord) // pass
                return { success: true, captures: []/*, ko: false*/ };

            if (this.getType(coord) != CLEAR)
                return {
                    success: false,
                    captures: undefined,
            ...
            for (var i = 0; i < adjacent.length; i++) {
                ...
                if (this.getType(c) == oppType) { // potential capture
                    ...
                    if (this.filter(g.neighbors, CLEAR).length === 1) {
            ...
            if (captures.length == 1 && this.filter(adjacent, CLEAR).length === 0
