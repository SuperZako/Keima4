...
namespace WGo {
        ...
        displayScore() {
            ...
            for (var i = 0; i < this.position.size; i++) {
                for (var j = 0; j < this.position.size; j++) {
                    let s = this.position.get(i, j);
                    let t = this.originalPosition.get(i, j);
    ...
    function territory_reset(pos, orig, x, y, margin) {
        var o = orig.get(x, y);
        if (pos.get(x, y) == o) {
            return;
        }
    ...
    if (WGo.BasicPlayer && WGo.BasicPlayer.component.Control) {
        WGo.BasicPlayer.component.Control.menu.push({
            constructor: function (player, args) { return new WGo.MenuItem(player, args) },
            args: {
                name: "scoremode",
                togglable: true,
