namespace WGo {
     ...
     *
     */
    function recursive_clone(node: KNode) {
    ...
    function find_property(prop, node: KNode) {
        var res;
    ...
    var recursive_save = function (gameTree, node: KNode) {
    ...
    function sgf_coordinates(x, y) {
        return String.fromCharCode(a_char + x) + String.fromCharCode(a_char + y);
    }

    function sgf_write_group(prop, values, output) {
        if (!values.length) {
            return;
        }
    ...
    var sgf_write_node = function (node, output) {
        ...
        if (node.setup) {
            ...
            for (var i in node.setup) {
                if (node.setup[i].c == WGo.B) {
                    ...
                    AE.push(sgf_coordinates(node.setup[i].x, node.setup[i].y));
                }
            }
        ...
        if (node.markup) {
            ...
            for (var i in node.markup) {
                ...
                if (node.markup[i].type == "LB")
                    markup["LB"].push(sgf_coordinates(node.markup[i].x, node.markup[i].y) + ":" + sgf_escape(node.markup[i].text));
        ...
        for (var key in props) {
            ...
            if (key == "turn") {
                ...
                output.sgf += key + "[" + sgf_escape(props[key]) + "]";
            }
        }
    ...
    export class Kifu {
        ...
        static infoList = ["black", "white", "AN", "CP", "DT", "EV", "GN", "GC", "HA", "ON", "OT", "RE", "RO", "RU", "SO", "TM", "US", "PC", "KM"];
        ...
        toSgf() {
            ...
            for (var key in this.info) {
                if (key == "black") {
                    ...
                    root_props[key] = sgf_escape(this.info[key]);
                }
            }
    ...
    i18n.en["show"] = "show";
    i18n.en["res-show-tip"] = "Click to show result.";
