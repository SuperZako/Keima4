...
namespace WGo {
     ...
     */
    export interface IBasicPlayerConfiguration {
    ...
    export class BasicPlayer extends Player {
        ...
        static defaultConfig: IBasicPlayerConfiguration = {
            layout: dynamicLayout,
        };
        static attributes = {
        ...
        components = {};
        dom;
        currentLayout: DynamicLayout;
        lastLayout: DynamicLayout;
        regions;
        info_overlay: HTMLDivElement;
        noresize: boolean;
        _editable;
        constructor(elem: HTMLDivElement, config) {
            super(elem, config);
            //this.config = config;

            // add default configuration of BasicPlayer
            for (var key in BasicPlayer.defaultConfig)
                if (this.config[key] === undefined && BasicPlayer.defaultConfig[key] !== undefined)
                    this.config[key] = BasicPlayer.defaultConfig[key];
        ...
        public updateDimensions() {
            ...
            if (tmp_w == this.width && tmp_h == this.height && tmp_mh == this.maxHeight) {
                return;
            }
            ...
            for (let i in this.components) {
                if (this.components[i].updateDimensions)
                    this.components[i].updateDimensions();
            }
        }
        private manageComponents() {
            // add detach flags to every widget
            for (var key in this.components) {
                this.components[key]._detachFromPlayer = true;
            }

            this.appendComponents("left");
            this.appendComponents("right");
            this.appendComponents("top");
            this.appendComponents("bottom");

            // detach all invisible components
            for (var key in this.components) {
                if (this.components[key]._detachFromPlayer && this.components[key].element.parentNode)
                    this.components[key].element.parentNode.removeChild(this.components[key].element);
            }
        }
        private appendComponents(area: string) {
            if (this.currentLayout.layout) {
                var components = this.currentLayout.layout[area];
            } else {
                var components = this.currentLayout[area];
            }
            ...
            if (components) {
                ...
                for (var i in components) {
                    if (!this.components[components[i]]) {
                        this.components[components[i]] = new BasicPlayer.component[components[i]](this);
                    }
        ...
        public showMessage(text: string, closeCallback?, permanent?) {
            ...
            if (closeCallback) {
                ...
                this.info_overlay.addEventListener("click", (e) => {
                    this.hideMessage();
                });
            }
        ...
        public showDialog(text: string, closeCallback?, permanent = true) {
            this.info_overlay = document.createElement("div");
            this.info_overlay.style.width = this.element.offsetWidth + "px";
            this.info_overlay.style.height = this.element.offsetHeight + "px";
            this.info_overlay.className = "wgo-info-overlay";
            ...
            let template = <HTMLTemplateElement>document.querySelector('#game-settings-dialog');
            var clone = document.importNode(template.content, true);
            let inputBlack = <HTMLInputElement>clone.querySelector("#radio-button-black");
            let ok_button = <HTMLButtonElement>clone.querySelector("#ok_button");
            ok_button.onclick = () => {
                this.hideMessage();

                this._editable = this._editable || new GameLogic(this, this.board, inputBlack.checked ? StoneColor.Black : StoneColor.White);
                this.setFrozen(true);
            ...
            this.info_overlay.appendChild(clone);
    ...
    i18n.en["BP:closemsg"] = "click anywhere to close this window";
    ...
    var player_from_tag = function (elem) {
        ...
        for (var a = 0; a < elem.attributes.length; a++) {
            ...
            if (BasicPlayer.attributes[att.name])
                BasicPlayer.attributes[att.name].call(config, att.value, att.name);
        }
