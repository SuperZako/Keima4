...
namespace WGo {
    ...
    export class FileError extends Error {
        constructor(path, code?: number) {
            ...
            if (code === 1) {
                ...
                this.message = "Network error. It is not possible to read '" + path + "'.";
            } else {
                this.message = "File '" + path + "' hasn't been found on server.";
            }
        }
    }
    ...
    var wheel_lis = function (e) {
        ...
        if (detect_scrolling(e.target, this)) {
            return true;
        }
    ...
    export class Player {
        ...
        _tmp_mark;
        temp_marks;
        public constructor(public element: HTMLElement, public config) {
            // add default configuration
            for (var key in Player.default) {
                if (this.config[key] === undefined && Player.default[key] !== undefined) {
                    this.config[key] = Player.default[key];
                }
            }
        ...
        init() {
            ...
            // creating listeners
            this.listeners = {
                kifuLoaded: [this.prepare_board],
                update: [this.update_board],
                frozen: [],
                unfrozen: [],
            };

            if (this.config.kifuLoaded) {
        ...
        update(op?) {
            if (!this.kifuReader || !this.kifuReader.change) {
                return;
            }
        ...
        loadSgfFromFile(file_path: string, game_path: string) {
            try {
                loadFromUrl(file_path, (sgf) => {
                    this.loadSgf(sgf, game_path);
                });
            }
            catch (err) {
                this.error(err);
        ...
        setWheel(b) {
            if (!this._wheel_listener && b) {
                ...
                this.element.addEventListener("mousewheel", this._wheel_listener);
            }
