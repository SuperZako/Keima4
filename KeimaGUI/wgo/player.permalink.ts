namespace WGo {
    ...
    window.addEventListener("hashchange", function () {
        if (window.location.hash != "" && permalink.active) {
            ...
            for (var key in permalink.query) {
                var p_el: any = document.getElementById(key);
                if (p_el && p_el._wgo_player)
                    p_el._wgo_player.goTo(move_from_hash);
            }
        }
    });
    ...
    if (WGo.BasicPlayer && WGo.BasicPlayer.component.Control) {
        WGo.BasicPlayer.component.Control.menu.push({
            constructor: (player, args) => new WGo./*BasicPlayer.control.*/MenuItem(player, args),
            args: {
                name: "permalink",
    ...
    (<any>WGo.Player).permalink = permalink;
