/// <reference path="./basicplayer.component.ts" />
namespace WGo {
    ...
    var format_info = function (info, title?) {
        ...
        if (title) ret += '<div class="wgo-info-title">' + t("gameinfo") + '</div>';
        for (var key in info) {
    ...
    export class CommentBox extends Component {
        ...
        constructor(private player: Player) {
            ...
            player.addEventListener("kifuLoaded", (e) => {
        ...
        setComments(e) {
            if (this.player._tmp_mark) {
                unmark.call(this.player);
            }
            var msg = "";
            if (!e.node.parent) {
        ...
        getCommentText(comment, formatNicks, formatMoves) {
            ...
            if (comment) {
                ...
                if (formatNicks)
                    comm = comm.replace(/(<p>)([^:]{3,}:)\s/g, '<p><span class="wgo-comments-nick">$2</span> ');
                if (formatMoves)
                    comm = comm.replace(/\b[a-zA-Z]1?\d\b/g, '<a href="javascript:void(0)" class="wgo-move-link">$&</a>');
                return comm;
            }
            return "";
        }
    ...
    WGo.BasicPlayer.defaultConfig.formatNicks = true;
    WGo.BasicPlayer.defaultConfig.formatMoves = true;
    ...
    layouts["right_top"].right.push("CommentBox");
    layouts["right"].right.push("CommentBox");
    layouts["one_column"].bottom.push("CommentBox");
