...
(function (Commands) {
    ...
    function getLastMove(n = 0) {
        let lastMove = Keima.Main.getLastMove(n);
        if (lastMove === "") {
            return null;
        }
    ...
    Commands.getLastMove = getLastMove;
    ...
    Commands.genmove = genmove;
    function clear_board() {
        Keima.Main.clearBoard();
    }
    Commands.clear_board = clear_board;
    function finalScore() {
        let result = Keima.Main.gets("final_score");
        let color = result.substr(0, 1) === "W" ? "White" : "Black";
        let point = Number(result.substr(1));
        return { color, point };
    }
    Commands.finalScore = finalScore;
})(Commands || (Commands = {}));
var util;
(function (util) {
    'use strict';
     ...
     */
    function getHandicapCoordinates(size, num) {
    ...
    util.getHandicapCoordinates = getHandicapCoordinates;
    /**
...
(function (JGO) {
    ...
    (function (BOARD) {
        BOARD.medium = {
            ...
            grid: {
                color: '#202020', x: 25, y: 25, smooth: 0,
            ...
            mark: {
                lineWidth: 1.0, blackColor: 'white', whiteColor: 'black',
                clearColor: 'black', font: 'normal 12px sanf-serif'
            }
        };
        BOARD.mediumWalnut = util.extend(util.extend({}, BOARD.medium), {
        ...
        BOARD.mediumBW = util.extend(util.extend({}, BOARD.medium), {
            textures: false
        });
    })(BOARD = JGO.BOARD || (JGO.BOARD = {}));
})(JGO || (JGO = {}));
/// <reference path="../util.ts" />
// Import or create JGO namespace
var JGO;
(function (JGO) {
    ...
    (function (BOARD) {
        BOARD.large = {
            ...
            // Grid color and size, line widths
            grid: {
                color: '#202020', x: 50, y: 50, smooth: 0.0,
...
(function (JGO) {
    ...
    class Board {
        ...
        getCoordinate(s) {
            ...
            return new JGO.Coordinate(JGO.COORDINATES.indexOf(s.toUpperCase().substr(0, 1)), parseInt(s.substr(1)));
        }
        /**
         ...
         */
        each(func, i1 = 0, j1 = 0, i2 = this.width - 1, j2 = this.height - 1) {
         ...
         */
        playMove(coord, stone, ko) {
            var oppType = (stone == JGO.BLACK ? JGO.WHITE : JGO.BLACK), captures = [], captured = {};
            if (!coord)
            ...
            let adjacent = this.getAdjacent(coord); // find adjacent coordinates
            for (var i = 0; i < adjacent.length; i++) {
                var c = adjacent[i];
                if (c.toString() in captured) {
                    continue; // avoid double capture
                }
            ...
            if (captures.length === 0 &&
                !this.hasType(this.getGroup(coord, stone).neighbors, /*C.*/ JGO.CLEAR)) {
                return {
                    success: false,
            ...
            if (captures.length == 1 && this.filter(adjacent, JGO.CLEAR).length === 0
...
(function (JGO) {
     ...
     */
    class Canvas {
        constructor(elem, opt, images) {
            this.listeners = { 'click': [], 'mousemove': [], 'mouseout': [] };
            /* global document */
            if (typeof elem === 'string')
                elem = document.getElementById(elem);
            var canvas = document.createElement('canvas'), i, j;
            let padLeft = opt.edge.left ? opt.padding.normal : opt.padding.clipped;
            let padRight = opt.edge.right ? opt.padding.normal : opt.padding.clipped;
            let padTop = opt.edge.top ? opt.padding.normal : opt.padding.clipped;
            let padBottom = opt.edge.bottom ? opt.padding.normal : opt.padding.clipped;
            ...
            this.getCoordinate = function (pageX, pageY) {
                ...
                return new JGO.Coordinate(Math.floor((scaledX - this.marginLeft - padLeft) / opt.grid.x) + opt.view.xOffset, Math.floor((scaledY - this.marginTop - padTop) / opt.grid.y) + opt.view.yOffset);
            }.bind(this);
            ...
            canvas.onmousemove = function (ev) {
                if (!this.listeners.mousemove.length)
                    return;
            ...
            this.stones = new JGO.Stones(opt, images);
            this.images = images;
            ...
            for (i = 0; i < opt.view.width; i++) {
                ...
                this.ctx.lineTo(smt + this.gridLeft + opt.grid.x * i, smt + this.gridTop + opt.grid.y * (opt.view.height - 1) +
            ...
            for (i = 0; i < opt.view.height; i++) {
                ...
                this.ctx.lineTo(smt + this.gridLeft + opt.grid.x * (opt.view.width - 1) +
                    (opt.edge.right ? 0 : opt.grid.x / 2 + padRight / 2), smt + this.gridTop + opt.grid.y * i);
                this.ctx.stroke();
            }
            if (opt.stars.points) {
                ...
                for (j = 0; j < 3; j++) {
                    for (i = 0; i < 3; i++) {
                        ...
                        this.ctx.beginPath();
                        this.ctx.arc(smt + this.gridLeft + x * opt.grid.x, smt + this.gridTop + y * opt.grid.y, opt.stars.radius, 0, 2 * Math.PI, false);
            ...
            for (i = 0; i < opt.view.width; i++) {
                if (opt.coordinates && opt.coordinates.top)
                    this.ctx.fillText(/*C.*/ JGO.COORDINATES[i + opt.view.xOffset], this.gridLeft + opt.grid.x * i, this.marginTop / 2);
                if (opt.coordinates && opt.coordinates.bottom)
                    this.ctx.fillText(/*C.*/ JGO.COORDINATES[i + opt.view.xOffset], this.gridLeft + opt.grid.x * i, canvas.height - this.marginBottom / 2);
            }
            // Draw vertical coordinates
            for (i = 0; i < opt.view.height; i++) {
                if (opt.coordinates && opt.coordinates.left)
                    this.ctx.fillText('' + (opt.board.height - opt.view.yOffset - i), this.marginLeft / 2, this.gridTop + opt.grid.y * i);
                if (opt.coordinates && opt.coordinates.right)
                    this.ctx.fillText('' + (opt.board.height - opt.view.yOffset - i), canvas.width - this.marginRight / 2, this.gridTop + opt.grid.y * i);
            }
            ...
            if (this.images.black)
                this.ctx.drawImage(this.images.black, 10, 10);
            if (this.images.white)
                this.ctx.drawImage(this.images.white, 10, 10);
            if (this.images.shadow)
                this.ctx.drawImage(this.images.shadow, 10, 10);
            // Sucks but works
        ...
        draw(jboard, i1, j1, i2, j2) {
            ...
            jboard.each(function (c, type, mark) {
                ...
                if (type == /*C.*/ JGO.CLEAR && mark && isLabel.test(mark))
                    clearFunc(ox, oy);
            ...
            jboard.each(function (c, type) {
                var ox = this.getX(c.i - this.opt.view.xOffset);
                var oy = this.getY(c.j - this.opt.view.yOffset);
                if (type == /*C.*/ JGO.BLACK || type == /*C.*/ JGO.WHITE) {
            ...
            jboard.each(function (c, type, mark) {
                ...
                switch (type) {
                    case JGO.BLACK:
                    case JGO.DIM_BLACK:
                        this.ctx.globalAlpha = type == /*C.*/ JGO.BLACK ? 1 : this.opt.stone.dimAlpha;
                        this.stones.drawStone(this.ctx, type, ox, oy);
                        markColor = this.opt.mark.blackColor; // if we have marks, this is the color
                        break;
                    case JGO.WHITE:
                    case JGO.DIM_WHITE:
                        this.ctx.globalAlpha = type == /*C.*/ JGO.WHITE ? 1 : this.opt.stone.dimAlpha;
                        this.stones.drawStone(this.ctx, type, ox, oy);
                ...
                this.ctx.textBaseline = 'middle';
                if (mark)
                    this.stones.drawMark(this.ctx, mark, ox, oy);
            }.bind(this), i1, j1, i2, j2); // provide iteration limits
...
(function (JGO) {
    ...
    class Setup {
        constructor(board, boardOptions) {
            ...
            this.notifier = new JGO.Notifier(this.board); // notifier to canvas
...
(function (JGO) {
    ...
    class Record {
        ...
        createNode(clearParentMarks, options) {
            var node = new JGO.RecordNode(this.jboard, this.current, options);
            if (clearParentMarks)
...
(function (Main) {
    ...
    let ko = undefined;
    let lastMove; // ko coordinate and last move coordinate
    ...
    Main.initializeValues = initializeValues;
    ...
    Main.setGameOn = setGameOn;
    function toggleGameOn() {
        setGameOn(!gameOn);
    }
    Main.toggleGameOn = toggleGameOn;
    function clearBoard() {
        let str = Keima.Main.gets("clear_board");
        jboard.clear();
    }
    Main.clearBoard = clearBoard;
    ...
    Main.setBoardSize = setBoardSize;
    ...
    Main.setPlayerColor = setPlayerColor;
    function resign() {
...
(function (JGO_1) {
    ...
    function parseMarkup(str) {
        ...
        for (var i = 0, len = lines.length; i < len; ++i) {
            ...
            for (var j = 0, len2 = line.length; j < len2; ++j) {
                switch (line[j]) {
                    case '.':
                        elems.push({ type: /*C.*/ JGO_1.CLEAR });
                        break;
                    case 'o':
                        elems.push({ type: /*C.*/ JGO_1.WHITE });
                        break;
                    case 'x':
                        elems.push({ type: /*C.*/ JGO_1.BLACK });
                        break;
                    case ' ':
                        break; // ignore whitespace
                    default:// assume marker
                        if (!elems.length)
                            break; // no intersection yet
    ...
    function process(JGO, div) {
        ...
        if (!width) {
            if (!data.length)
                return; // no size or data, no board
            height = data.length;
            width = data[0].length;
        }
...
(function (JGO) {
     ...
     */
    class RecordNode {
        constructor(jboard, parent, info) {
            this.jboard = jboard;
            this.parent = parent;
            this.children = [];
            this.changes = [];
        ...
        clearParentMarks() {
            ...
            for (var i = this.parent.changes.length - 1; i >= 0; i--) {
                ...
                if ('mark' in item)
                    this.setMark(item.c, /*C.*/ JGO.MARK.NONE);
            }
        }
        /**
...
(function (JGO) {
    ...
    function sgfMove(node, name, values, moveMarks) {
        ...
        if (name == 'B') {
            player = /*C.*/ JGO.BLACK;
            opponent = /*C.*/ JGO.WHITE;
        }
        else if ('W') {
            player = /*C.*/ JGO.WHITE;
            opponent = /*C.*/ JGO.BLACK;
        }
        ...
        if (moveMarks && play.ko)
            node.setMark(play.ko, /*C.*/ JGO.MARK.SQUARE);
        if (play.success && coord !== null) {
            node.setType(coord, player); // play stone
            node.setType(play.captures, /*C.*/ JGO.CLEAR); // clear opponent's stones
            if (moveMarks)
                node.setMark(coord, /*C.*/ JGO.MARK.CIRCLE);
        }
        else
            ERROR = play.error;
        return play.success;
    ...
    function sgfSetup(node, name, values) {
        var setupMap = { 'AB': /*C.*/ JGO.BLACK, 'AW': /*C.*/ JGO.WHITE, 'AE': /*C.*/ JGO.CLEAR };
        node.setType(explodeSGFList(values), setupMap[name]);
        return true;
    }
    function sgfMarker(node, name, values) {
        var markerMap = {
            'TW': /*C.*/ JGO.MARK.WHITE_TERRITORY,
            'TB': /*C.*/ JGO.MARK.BLACK_TERRITORY,
            'CR': /*C.*/ JGO.MARK.CIRCLE,
            'TR': /*C.*/ JGO.MARK.TRIANGLE,
            'MA': /*C.*/ JGO.MARK.CROSS,
            'SQ': /*C.*/ JGO.MARK.SQUARE
        };
    ...
    function gameTreeToRecord(gameTree, moveMarks) {
        var root = gameTree.sequence[0], width = 19, height = 19;
        if ('SZ' in root) {
            var size = root.SZ[0];
            if (size.indexOf(':') != -1) {
        ...
        let jrecord = new JGO.Record(width, height);
...
(function (JGO) {
    ...
    class Stones {
        ...
        drawStone(ctx, type, ox, oy, scale) {
            if (!scale)
                scale = 1;
            var stone = (type == /*C.*/ JGO.BLACK || type == /*C.*/ JGO.DIM_BLACK) ? this.images.black : this.images.white;
            if (!stone) {
                ctx.fillStyle = (type == /*C.*/ JGO.WHITE) ? '#FFFFFF' : '#000000';
                ctx.beginPath();
        ...
        drawMark(ctx, mark, ox, oy) {
            switch (mark) {
                ...
                case JGO.MARK.BLACK_TERRITORY:
                    ctx.globalAlpha = 1;
                    this.drawStone(ctx, /*C.*/ JGO.BLACK, ox, oy, 0.5);
                    break;
                case JGO.MARK.WHITE_TERRITORY:
                    ctx.globalAlpha = 1;
                    this.drawStone(ctx, /*C.*/ JGO.WHITE, ox, oy, 0.5);
                    break;
...
class PassDialog extends Dialog {
    constructor() {
        ...
        this.content = document.getElementById("pass-dialog-content");
...
class ResultDialog extends Dialog {
    constructor() {
        ...
        this.content = document.getElementById("result-dialog-content");
        this.okButton = document.getElementById("result-dialog-ok");
...
class SettingsDialog extends Dialog {
    constructor() {
        ...
        this.okButton.addEventListener("click", () => {
            super.hide();
            var boardSize = document.getElementsByName("board-size");
            let size = 19;
