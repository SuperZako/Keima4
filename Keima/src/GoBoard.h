...
constexpr int PURE_BOARD_SIZE = 19;  // 盤の大きさ

constexpr int OB_SIZE = 5; // 盤外の幅
constexpr int BOARD_SIZE = (PURE_BOARD_SIZE + OB_SIZE + OB_SIZE); // 盤外を含めた盤の幅

constexpr int PURE_BOARD_MAX = (PURE_BOARD_SIZE * PURE_BOARD_SIZE); // 盤の大きさ 
constexpr int BOARD_MAX = (BOARD_SIZE * BOARD_SIZE);                // 盤外を含めた盤の大きさ

constexpr int MAX_STRING = (PURE_BOARD_MAX * 4 / 5); // 連の最大数 
constexpr int MAX_NEIGHBOR = MAX_STRING;             // 隣接する敵連の最大数

constexpr int BOARD_START = OB_SIZE;                        // 盤の始点  
constexpr int BOARD_END = (PURE_BOARD_SIZE + OB_SIZE - 1);  // 盤の終点  

constexpr int STRING_LIB_MAX = (BOARD_SIZE * (PURE_BOARD_SIZE + OB_SIZE));  // 1つの連の持つ呼吸点の最大数
constexpr int STRING_POS_MAX = (BOARD_SIZE * (PURE_BOARD_SIZE + OB_SIZE));  // 連が持ちうる座標の最大値

constexpr int STRING_END = (STRING_POS_MAX - 1); // 連の終端を表す値
constexpr int NEIGHBOR_END = (MAX_NEIGHBOR - 1);  // 隣接する敵連の終端を表す値
constexpr int LIBERTY_END = (STRING_LIB_MAX - 1); // 呼吸点の終端を表す値

constexpr int MAX_RECORDS = (PURE_BOARD_MAX * 3); // 記録する着手の最大数 
constexpr int MAX_MOVES = (MAX_RECORDS - 1);      // 着手数の最大値

constexpr int PASS = 0;     // パスに相当する値
constexpr int RESIGN = -1;  // 投了に相当する値

constexpr double KOMI = 6.5; // デフォルトのコミの値
